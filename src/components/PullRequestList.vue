<template>
  ...
  <div id="pull-request">
    <div class="filters" style="display:flex">
      ...
      <button
        class="button"
        v-on:click="visibility = 'waiting'"
        :class="{ selected: visibility === 'waiting' }"
      >
        Waiting
      </button>
      <button
        class="button"
        v-on:click="visibility = 'merged'"
        :class="{ selected: visibility === 'merged' }"
      >
        Merged
      </button>
    </div>
    ...
    <div
      style="width:510px; height:350px; overflow-x:hidden; overflow-y: auto;"
    >
      <div
      ...
      >
        <div
          class="title"
          v-bind:class="{ titleblur: pullrequest.water_status === 'Watered' }"
        >
          <img
            class="status"
            :src="
              pullrequest.collect_status === 'Collected' &&
              pullrequest.status !== branch_merged
                ? require('../assets/waiting.png')
                : pullrequest.status_src
            "
          />
        ...
        <div class="buttons">
          <div>
            <b-button
              @click="$bvModal.show(getModalId(pullrequest.id))"
              class="collectbutton"
              v-bind:class="{ [pullrequest.collect_status]: true }"
            >
              Collect
            </b-button>
            <b-modal
              :id="getModalId(pullrequest.id)"
              size="sm"
            ...
            >
              <h6>
                You have earned
                {{ plantRepresentation[pullrequest.reward].name }} seed!
              </h6>
              ...
              <template slot="modal-footer" slot-scope="{ ok }">
                <b-button
                  ...
                  class="Collect"
                  size="m"
                  @click="$bvModal.hide(getModalId(pullrequest.id))"
                  v-on:click="collectItem(pullrequest)"
                >
                  Let's grow the plant!
                </b-button>
              </template>
            </b-modal>
          </div>
          <div>
            <button
              ...
              class="collectbutton disabled"
            >
              Water
            </button>
            <button
              v-else
              v-on:click="waterForest(pullrequest)"
              class="collectbutton"
              v-bind:class="pullrequest.water_status"
            >
              Water
            </button>
          </div>
...
export default {
  ...
  computed: {
    ...
    filteredPullRequests: function() {
      return filters[this.visibility](this.pullRequests).map(pr => ({
        ...pr,
        status_src: displayableByBranchStatus[pr.status].status_src
      }));
    },
